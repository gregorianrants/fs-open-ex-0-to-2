...
const App = () => {
  ...
  function handleRemovePerson(id) {
    ...
    let shouldDelete = window.confirm(
      `Delete ${personState.getById(id).name} ?`
    );
    ...
    personsService
      ...
      .catch((err) => {
        setWarningMessage(
          `Information of ${name} has already been removed from the phone book`
        );
        let newPersons = persons.filter((person) => person.id != id);
        setPersons(newPersons);
      });
      ...
      personsService
        ...
        .catch((err) => {
          ...
          if (
            ...
            "There is no person with the provided id"
          ) {
            setWarningMessage(
              `Information of ${existingPerson.name} has previously been removed from phone book, this will now be removed from the screen`
            );
          } else {
            setWarningMessage(
              err?.response?.data?.error || "something went very wrong"
            );
          }
  ...
  function handleFormSubmit(e) {
    e.preventDefault();
    if (personState.nameAlreadyExists(newName))
      return handleFormSubmit.existingName();
    handleFormSubmit.newName();
  }
